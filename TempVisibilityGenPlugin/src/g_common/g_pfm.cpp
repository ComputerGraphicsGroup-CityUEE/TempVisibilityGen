...
void GPfm::load( int width, int height, const DOUBLE3 *dm )
{
  if( w!=width || h!=height )
    load( width, height );
  ...
  for( j=0; j<h; j++ )
    for( i=0; i<w; i++ )
    {
      fm[j*w+i] = FLOAT3( (float)dm[j*w+i].x, (float)dm[j*w+i].y, (float)dm[j*w+i].z );
    }
}
...
void GPfm::drawline( float x0, float y0, float x1, float y1, FLOAT3 col )
{
  int i0, i1, j0, j1;
  float di, dj;
  int npix, s;
  ...
  j0 = g_round(y0*h);
  j1 = g_round(y1*h);
  npix = g_max( abs(i1-i0) , abs(j1-j0) );
  di = float(i1-i0) / npix;
  dj = float(j1-j0) / npix;
  // for( s=0; s<=npix; s++ )
  //   pm[g_clamp(g_round( j0+s*dj ),0,h-1)][g_clamp(g_round( i0+s*di ),0,w-1)] = col;
  for( s=0; s<=npix; s++ )
  {
    int ix, iy;
    ix = g_round( i0+s*di ); 
...
void GPfm::drawlineX( int x0, int y0, int x1, int y1, FLOAT3 col )
{
  int i0, i1, j0, j1;
  float di, dj;
  int npix, s;
...
void GPfm::drawrect( float nx0, float ny0, float mx1, float my1, FLOAT3 col )
{
  if(nx0>mx1)
    g_swap(nx0,mx1);
  if(ny0>my1)
  ...
  int i0, i1, j0, j1;
  int i, j;
  i0 = g_clamp( g_round(nx0*w), 0, w-1 );
  i1 = g_clamp( g_round(mx1*w), 0, w-1 );
  j0 = g_clamp( g_round(ny0*h), 0, h-1 );
  j1 = g_clamp( g_round(my1*h), 0, h-1 );
  for( j=j0; j<=j1; j++ )
  for( i=i0; i<=i1; i++ )
    pm[j][i] = col;
}
...
void GPfm::save( const char *spath, const char *_mode ) const
{
  char mode[256];
  if( _mode )
    strcpy(mode,_mode);
...
void GPf4::load( const char *spath )
{
  ...
  if( strcmp(gp.ename,"png")==0 )
  {
    ...
    for( i=0; i<w*h; i++ )
    {
      fm[i] = (*(BYTE4*)&png.bm[4*i]);
      fm[i] /= 255;
    }
    return;
  }
  if( strcmp(gp.ename,"ppm")==0 || strcmp(gp.ename,"jpg")==0 || strcmp(gp.ename,"pfm")==0 )
  {
    GPfm img;
    img.load(spath);
    load( img.w, img.h, img.fm );
    return;
  }
...
void GPf4::save( const char *spath ) const
{
  ...
  }else
  {
    ...
    int i;
    for( i=0; i<w*h; i++ )
      pfm.fm[i] = FLOAT3( fm[i].x, fm[i].y, fm[i].z );
    pfm.save( spath, gp.ename );
  }
}
...
void GPf4::load( int width, int height, const float *a )
{
  load( width, height );
...
void GPng::load( const char *spath, int num_channel )
{
  ...
  }else
  {
    ...
    do
    {
      ...
      switch( chk_id )
      {
        case 'IHDR':
          ...
          if( ihdr.Colour!=2 && ihdr.Colour!=3 && ihdr.Colour!=4 && ihdr.Colour!=6 )
          {
    ...
    if( ihdr.Interlace==0 )
    {
      ...
      if( ihdr.Colour==4 )
      {
        load( ihdr.Width, ihdr.Height, num_channel==0 ? 4 : num_channel );
        for( j=0; j<h; j++ )
        {
          trow = &des[(w*nb+1)*j];
          type = trow[0];
...
void GPng::drawrow( unsigned char *des, int db, const unsigned char *src, int sb, int w )
{
  ...
  }else if( db==3 && sb==2 )
  {
    ...
    {
      s = float(ts[1])/255;
      td[0]=td[1]=td[2] = ftoi((float(ts[0])/255)*s,255);
    }
  }else if( db==3 && sb==4 )
  {
    for( i=0, td=des, ts=src; i<w; i++, td+=db, ts+=sb )
...
#ifdef _DEBUG
#pragma comment (lib, "../../lib/zlib/lib/x64/zlib.lib")  // Link with zlib in Debug Mode
#endif
#ifdef NDEBUG
#pragma comment (lib, "../../lib/zlib/lib/x64/zlib.lib")  // Link with zlib in Release Mode
#endif
...
#ifdef _DEBUG
#pragma comment (lib, "../../lib/zlib/lib/zlib.lib")  // Link with zlib in Debug Mode
#endif
#ifdef NDEBUG
...
#pragma comment (lib, "../../lib/JpegLib/lib/x64/Release/libjpeg.lib")  // Link with JpegLib in Release Mode
#else
...
void GPdm::save( const char *spath )
{
  ...
  GPath gp = parse_spath(spath);
  src.save( spath, gp.ename );
}

void GPdm::flip_vertical()
{
  DOUBLE3 *tline, *line0, *line1;
    tline = (DOUBLE3*) malloc( w * sizeof(DOUBLE3) );
    line0 = fm;
    line1 = fm + w*(h-1);
  ...
  for( j=0; j<jlen; j++, line0+=w, line1-=w )
  {
    memcpy( tline, line0, w*sizeof(DOUBLE3) );
    memcpy( line0, line1, w*sizeof(DOUBLE3) );
    memcpy( line1, tline, w*sizeof(DOUBLE3) );
  }

  free(tline);
}

DOUBLE3 GPdm::lookup_linear( double sx, double sy ) const
{
  double qx, qy;
  double rx, ry;
    qx = floor(sx-.5)+.5;
...
void GPdm::load( int width, int height, const DOUBLE3 *src )
{
  load(width,height);
  memcpy( fm, src, w*h*sizeof(DOUBLE3) );
}
...
void GPd4::load( int width, int height )
{
  SAFE_FREE( fm );
  SAFE_FREE( pm );
  ...
  w = width;
  h = height;
  fm = (DOUBLE4*) malloc( w * h * sizeof(DOUBLE4) );

  if(fm==0)
  {
    printf( "%i %i %i\n", w * h * sizeof(DOUBLE4), w, h );
  }

  memset( fm, 0, w*h*sizeof(DOUBLE4) );

  pm = (DOUBLE4**) malloc( h * sizeof(DOUBLE4*) );
  int j;
  for( j=0; j<h; j++ )
    pm[j] = &fm[j*w];
}

void GPd4::load( const char *spath )
{
  GPfm src;
  src.load( spath );
  load( src.w, src.h, src.fm );
...
void GPd4::save( const char *spath )
{
  GPfm src;
  int i;
...
void GPd4::load( int width, int height, const FLOAT3 *src )
{
  int i;
  load(width,height);
  for( i=0; i<w*h; i++ )
...
void GPd4::load( int width, int height, const DOUBLE4 *src )
{
  load(width,height);
  memcpy( fm, src, w*h*sizeof(DOUBLE4) );
}

GPd4::GPd4()
{
  memset( this, 0, sizeof(GPd4) );
}
...
void GPd4::draw( const GPd4 &blk, int x, int y)
{
  int src_w, src_h;
    src_w = blk.w;
    src_h = blk.h;
  ...
  int j;
  for( j=0; j<src_h; j++, src+=src_w, des+=des_w )
    memcpy( des, src, src_w * sizeof(DOUBLE4) );
}

void GPd4::getblk( GPd4 &blk, int x, int y, int blkw, int blkh ) const
{
  if( blk.w != blkw || blk.h != blkh )
    blk.load( blkw,blkh );
  ...
  for( j=0; j<blkh; j++ )
    memcpy( blk.pm[j], &pm[y+j][x], blkw * sizeof(DOUBLE4) );
}

void GPd4::mul( double a )
{
  int t, l;
  DOUBLE4 *tm;
...
void GPd4::add( const GPd4 &A )
{
  if( w!=A.w || h!=A.h )
  {
    printf( "[Error] : GPd4::add(), the GPd4 are not of the same dimension.\n" );
    exit(-1);
  }
...
DOUBLE4 GPd4::lookup_linear( double sx, double sy ) const
{
  double qx, qy;
  double rx, ry;
    qx = floor(sx-.5)+.5;
...
void GPd4::flip_vertical()
{
  DOUBLE4 *tline, *line0, *line1;
    tline = (DOUBLE4*) malloc( w * sizeof(DOUBLE4) );
    line0 = fm;
    line1 = fm + w*(h-1);
  ...
  for( j=0; j<jlen; j++, line0+=w, line1-=w )
  {
    memcpy( tline, line0, w*sizeof(DOUBLE4) );
    memcpy( line0, line1, w*sizeof(DOUBLE4) );
    memcpy( line1, tline, w*sizeof(DOUBLE4) );
  }
