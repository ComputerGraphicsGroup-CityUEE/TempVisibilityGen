...
{
  char dname[256];		//path to the file
  char fname[256];		//file name
  char ename[256];		//file type
}GPath;
...
template <typename T> T g_lerpd( const T &a, const T &b, double s )
{
  s = g_clamp(s,0.0,1.0);
  return (1-s)*a+s*b;  
}
...
template <typename T> T g_sqr( const T &x ){ return x*x; }
