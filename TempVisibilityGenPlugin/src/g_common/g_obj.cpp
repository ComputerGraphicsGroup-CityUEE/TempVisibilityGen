...
void GObj::load( const char *_spath, int tex_size )
{
  char spath[256];
  if( _spath )
    strcpy(spath,_spath);
  else
    strcpy(spath,obj_path);
  clear();
  ...
  if( strcmp( gp->ename, "m" )==NULL )
  {
    loadx( spath );
    return;
  }else if( strcmp( gp->ename, "wrl" )==NULL )
  {
    openwrl( spath, tex_size );
    return;
  }
  ...
  if( n_texc )
  {
    ...
    for( i=0; i<sizeof(ext)/sizeof(ext[0]); i++ )
    {
      ...
      if( fexist( tex_path ) )
      {
        pfm.load( tex_path );
        if(tex_size!=0)
          pfm.resample( tex_size, tex_size );
        pfm.flip_vertical();
        break;
      }
    }
...
void GObj::load_mtl( const char *_spath, int tex_size )
{
  char spath[256];
  if(_spath)
    strcpy(spath,_spath);
  else
    strcpy(spath,mtl_path);
    ...
    FILE *f0 = fopen( spath, "rb" );
      while( fgets( str, 256, f0 ) )
      {
        ...
        if( sscanf( str0, "%s", tag )==1 )
        {
          ...
          }else if( strcmp( tag, "bump" )==0 )
          {
            sscanf( str0, "bump %f", &mtl->bump );
            mtl->ward.w = 1;
...
void GObj::setmtl( const char *id, const Material &mtl, int tex_size )
{
  int i;
  for( i=0; i<n_material; i++ )
  {
    ...
    {
      if( strcmp(material[i].texname,mtl.texname) )
      {
        if( material[i].texid )
          glDeleteTextures( 1, &material[i].texid );

        char texpath[256];
        GPath gp = parse_spath( obj_path );
        sprintf( texpath, "%s%s", gp.dname, mtl.texname );

        memcpy( &material[i], &mtl, sizeof(Material) );
        ...
        glGenTextures( 1, &material[i].texid );
  ...
  material = (Material*) realloc( material, (n_material+1) * sizeof(Material) );
  Material mtl1 = getmtl("");
  strcpy( mtl1.name, id );
  material[ n_material ] = mtl1;
  n_material+=1;
}
...
GObj::Material GObj::getmtl( const char *id ) const
{
  ...
  Material default_mtl;
    memset( &default_mtl, 0, sizeof(Material) );
    default_mtl.ambient   = 0;
    default_mtl.diffuse   = 1;
    default_mtl.specular  = 1;
    default_mtl.shininess = 8;
    default_mtl.bump = 0;
    default_mtl.illum = 2;
    default_mtl.texid = 0;
  return default_mtl;
...
{
  FLOAT4 bdcol = FLOAT4( mtl.diffuse.x*.5f, mtl.diffuse.y*.5f, mtl.diffuse.z, 1 );
  float zero[4] = {0, 0 , 0 , 1};

  glMaterialfv( GL_FRONT, GL_AMBIENT, (float*)&mtl.ambient   );
  glMaterialfv( GL_BACK , GL_AMBIENT, zero   );

  if( mtl.illum>=1 )
  {
    glMaterialfv( GL_FRONT, GL_DIFFUSE,   (float*)&mtl.diffuse   );
    glMaterialfv( GL_BACK , GL_DIFFUSE,   (float*)&bdcol   );
  }else
    glMaterialfv( GL_FRONT_AND_BACK, GL_DIFFUSE,   zero  );

  if( mtl.illum==2 )
  {
    glMaterialfv( GL_FRONT, GL_SPECULAR,  (float*)&mtl.specular  );
    glMaterialfv( GL_BACK , GL_SPECULAR,  zero   );
  }else
    glMaterialfv( GL_FRONT_AND_BACK, GL_SPECULAR,  zero  );

  glMaterialf(  GL_FRONT_AND_BACK, GL_SHININESS, g_clamp(mtl.shininess,0.f,128.f) );
}

void GObj::gen_vertex_array( const Group &grp, int &ndat, float* &dat ) const
{
  int i, s;
  int t0;

  ndat = (3*sizeof(float) + 2*sizeof(float) + 3*sizeof(float) + 3*sizeof(float))*3*grp.n_face;
  dat = (float*) malloc( ndat );
  float *val;

  for( i=0, val=dat; i<grp.n_face; i++ )
  {
    t0 = grp.tri[i];
    for( s=0; s<3; s++ )
    {
      *((FLOAT3*)val) =  vertex.fm[ face_vidx[t0*3+s] ];   val+=3;
      *val            =    texc.fm[ face_tidx[t0*3+s] ].x; val+=1;
      *val            =    texc.fm[ face_tidx[t0*3+s] ].y; val+=1;
      *((FLOAT3*)val) =  normal.fm[ face_nidx[t0*3+s] ];   val+=3;
      *((FLOAT3*)val) = tangent.fm[ face_nidx[t0*3+s] ];   val+=3;
    }
  }
}
...
void GObj::draw_group( const Group &grp, int draw_option ) const
{
  ...
  Material mtl;
    mtl = getmtl( grp.name );
  if( !(draw_option & GOBJ_NO_MATERIAL) )
    usemtl( mtl );
  ...
  int t0;
    ...
    if( (grp.type & draw_option)==GOBJ_VTNT && n_texc && n_normal && tangent.w )
    {
      if( !(draw_option & GOBJ_NO_MATERIAL) )
      {
        if( mtl.texid )
      ...
      {
        t0 = grp.tri[i];
        //glMultiTexCoord3fv( GL_TEXTURE1, (float*)&tangent.fm[ face_nidx[t0*3  ] ] );
        ...
        //glMultiTexCoord3fv( GL_TEXTURE1, (float*)&tangent.fm[ face_nidx[t0*3+1] ] );
        ...
        //glMultiTexCoord3fv( GL_TEXTURE1, (float*)&tangent.fm[ face_nidx[t0*3+2] ] );
    ...
    }else if( (grp.type & draw_option)==GOBJ_VTN && n_texc && n_normal )
    {
      if( !(draw_option & GOBJ_NO_MATERIAL) )
      {
      if( mtl.texid )
      ...
      for( i=0; i<grp.n_face; i++ )
      {
        t0 = grp.tri[i];
        glNormal3fv( (float*)&normal.fm[ face_nidx[t0*3  ] ] );
        glTexCoord3fv( (float*)&texc.fm[ face_tidx[t0*3  ] ] );
        glVertex3fv( (float*)&vertex.fm[ face_vidx[t0*3  ] ] );
        glNormal3fv( (float*)&normal.fm[ face_nidx[t0*3+1] ] );
        glTexCoord3fv( (float*)&texc.fm[ face_tidx[t0*3+1] ] );
        glVertex3fv( (float*)&vertex.fm[ face_vidx[t0*3+1] ] );
        glNormal3fv( (float*)&normal.fm[ face_nidx[t0*3+2] ] );
        glTexCoord3fv( (float*)&texc.fm[ face_tidx[t0*3+2] ] );
        glVertex3fv( (float*)&vertex.fm[ face_vidx[t0*3+2] ] );
      }
      glEnd();
...
unsigned int GObj::list( int draw_option )
{
  GLuint list;
  list = glGenLists(1);
...
unsigned int GObj::list_by_group( int draw_option )
{
  GLuint list;
  int j;
  list = glGenLists(n_mgroup);
  for( j=0; j<n_mgroup; j++ )
  {
    glNewList(list+j, GL_COMPILE);
    draw_group( mgroup[j], draw_option );
    glEndList();
  }
...
FLOAT3 GObj::get_face_normal( int face_index ) const
{
  FLOAT3 v0, v1, v2;
  FLOAT3 fn;
...
FLOAT3 GObj::get_face_normal( FLOAT3 v0, FLOAT3 v1, FLOAT3 v2 ) const
{
...
void GObj::calculate_vertex_normal( float angle )
{
  ...
  for( j=0; j<n_vertex; j++ )
  {
    ...
    for( i=0; i<il; i++ )
    {
      ...
      for( k=k0; k<gs.ns; k++ )
      {
        bdot = vdot( n, vnormalize(gs.buf[k]) );
...
void GObj::calculate_tangent_space()
{
  ...
  for( j=0; j<n_normal; j++ )
  {
    ...
    if( zn.norm() < .00001 )
    {
      ...
      while(zn.norm()==0)
      {
        zn = FLOAT3( float(rand()), float(rand()), float(rand()) )/RAND_MAX;
        zn = zn - vdot(zn,yn)*yn;
      }
    }
...
void GObj::at_least_one_mgroup()
{
  if( n_mgroup==0 )
  {
    ...
    for( i=0; i<n_face; i++ )
      mgroup[0].add(i);
    mgroup[0].type = default_group.type;
  }
}

bool findchar( const char *seq, int l, char c )
{
  int i;
  for( i=0; i<l; i++ )
    if(c==seq[i])
...
int findlastchar( char *src, int len, int p0, const char *seq0, const char *seq1, char *buf )
{
  int j;
  for( j=p0; j<len; j++ )
  {
    if( findchar(seq0, strlen(seq0), src[j]) )
      break;
  }
  if(buf)
  {
    memcpy( buf, &src[p0], (j-p0)*sizeof(char) );
    buf[j-p0]=0;
  }
  for(  ; j<len && findchar(seq1,strlen(seq1),src[j]); j++ )
  {
  }
  if(j-p0==0)
  {
    buf[0] = src[p0];
    buf[1] = 0;
    for( j=p0+1; j<len && findchar(seq1,strlen(seq1),src[j]); j++ )
    {
    }
  }
  return j-p0;
}
void fprintspace( FILE *f0, int n )
{
  int i;
  for( i=0; i<n; i++ )
    fprintf( f0, " " );
...
void GObj::openwrl( const char *spath, int tex_size )
{
  int len = fsize(spath);
  char *src = (char*) freadall(spath);
  char *buf = (char*) malloc( (len+1)*sizeof(char) );

  // FILE *f0 = fopen( "tmp.txt", "wt" );
  int indent = 0;
  vector<FLOAT3> *point = NULL;
  vector<FLOAT3> coord;
  ...
  for( i=0; i<len; )
  {
    if(src[i]=='#')
    {
      int l = findlastchar(src,len,i,"\r\n"," \r\n",buf);
      i+=l;
    }else if(src[i]=='\"')
    {
      i += findlastchar(src,len,i,"\""," \r\n",buf);
      i += findlastchar(src,len,i,"\""," \r\n",buf);
      i += findlastchar(src,len,i,"\""," \r\n",buf);
    }else
    {
      ...
      if( strcmp("point", buf) == 0 )
      {
        if(point)
          point->clear();
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        i += findlastchar(src,len,i,"\"{}[],\r\n"," ,\r\n",buf);
        while( i<len && strcmp(buf,"]")!=0 )
        {
          FLOAT3 val;
          int nval = sscanf( buf, "%f %f %f", &val.x, &val.y, &val.z );
          if(point)
            point->push_back(val);
          i += findlastchar(src,len,i, "\"{}[],\r\n" ," ,\r\n", buf);
        };
        //fprintspace( f0, indent*2 );
        //if(point)
        //  fprintf( f0, "point[%i]\n", point->size() );

      }else if( strcmp("coordIndex", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        i += findlastchar(src,len,i,"\"{}[],\r\n"," ,\r\n",buf);
        while( i<len && strcmp(buf,"]")!=0 )
        {
          coordIndex.push_back( atoi(buf) );
          i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        };
        //fprintspace( f0, indent*2 );
        //fprintf( f0, "coordIndex[%i]\n", coordIndex.size() );

      }else if( strcmp("texCoordIndex", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        while( i<len && strcmp(buf,"]")!=0 )
        {
          texCoordIndex.push_back( atoi(buf) );
          i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        };
        //fprintspace( f0, indent*2 );
        //fprintf( f0, "texCoordIndex[%i]\n", texCoordIndex.size() );

      }else if( strcmp("DEF", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
      }else if( strcmp("appearance", buf) == 0 )
      {
        i += findlastchar(src,len,i,"{"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "appearance\n" );

      }else if( strcmp("material", buf) == 0 )
      {
        i += findlastchar(src,len,i,"{"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "material\n" );

      }else if( strcmp("texture", buf) == 0 )
      {
        i += findlastchar(src,len,i,"{"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "texture\n" );

      }else if( strcmp("geometry", buf) == 0 )
      {
        i += findlastchar(src,len,i,"{"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "geometry\n" );
      ...
      }else if( strcmp("ccw", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "ccw %s\n", strcmp(buf,"TRUE")==0 ? "true" : "false" );

      }else if( strcmp("solid", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "solid %s\n", strcmp(buf,"TRUE")==0 ? "true" : "false" );

      }else if( strcmp("url", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\""," \r\n",buf);
          i += findlastchar(src,len,i,"\""," \r\n",buf);
          //fprintspace(f0,indent*2);
      ...
      }else if( strcmp("translation", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        translationv.x = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        translationv.y = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        translationv.z = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "translation %f %f %f\n", pos.x, pos.y, pos.z );

      }else if( strcmp("rotation", buf) == 0 )
      {
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        axis_angle.x = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        axis_angle.y = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        axis_angle.z = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        axis_angle.w = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "translation %f %f %f\n", pos.x, pos.y, pos.z );

      }else if( strcmp("diffuseColor", buf) == 0 )
      {
        FLOAT3 col;
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        col.x = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        col.y = (float)atof(buf);
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        col.z = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "diffuseColor %f %f %f\n", col.x, col.y, col.z );

      }else if( strcmp("specularColor", buf) == 0 )
      {
        FLOAT3 col;
          i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
          col.x = (float)atof(buf);
          ...
          col.z = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "specularColor %f %f %f\n", col.x, col.y, col.z );

      }else if( strcmp("ambientIntensity", buf) == 0 )
      {
        float ambientIntensity;
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        ambientIntensity = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "ambientIntensity %f\n", ambientIntensity );

      }else if( strcmp("shininess", buf) == 0 )
      {
        float shininess;
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        shininess = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "shininess %f\n", shininess );

      }else if( strcmp("transparency", buf) == 0 )
      {
        float transparency;
        i += findlastchar(src,len,i,"\"{}[] ,\r\n"," ,\r\n",buf);
        transparency = (float)atof(buf);
        //fprintspace(f0,indent*2);
        //fprintf( f0, "transparency %f\n", transparency );
  ...
  //printf( "texCoord %i\n", texCoord.size() );
  //printf( "coordIndex %i\n", coordIndex.size() );
  //printf( "texCoordIndex %i\n", texCoordIndex.size() );

  //GObj obj;
  this->clear();
  if(coord.size())
  {
    this->load_vertex(coord.size());
    for( i=0; i<(int)coord.size(); i++ )
      this->vertex.fm[i] = coord[i];
    if(texCoord.size())
    {
      this->load_texc(texCoord.size());
      for(i=0; i<(int)texCoord.size(); i++)
        this->texc.fm[i] = texCoord[i];
    }
    this->load_face(coordIndex.size()/4);
    for( i=0; i<this->n_face; i++ )
    {
      this->face_vidx[3*i+0] = coordIndex[4*i+0];
      this->face_vidx[3*i+1] = coordIndex[4*i+1];
      this->face_vidx[3*i+2] = coordIndex[4*i+2];
      if(texCoord.size())
      {
        this->face_tidx[3*i+0] = texCoordIndex[4*i+0];
        this->face_tidx[3*i+1] = texCoordIndex[4*i+1];
        this->face_tidx[3*i+2] = texCoordIndex[4*i+2];
      }
      this->default_group.add(i);
    }
    this->default_group.type  = GOBJ_V__;
    if(texCoord.size())
      this->default_group.type |= GOBJ_TEXCOR;
this->weld();
    this->calculate_face_normal();
    this->rotate( axis_angle.w*180/G_PI, axis_angle.x, axis_angle.y, axis_angle.z );
    this->translate(translationv.x,translationv.y,translationv.z);
  ...
  //char obj_path[256];
  //sprintf( obj_path, "%s%s.obj", gp.dname, gp.fname );
  //this->save( obj_path );
}
