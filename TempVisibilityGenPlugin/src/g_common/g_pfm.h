...
class GPfm
{
  public:
    ...
    void load( int width, int height, const DOUBLE3 *dm );
    ...
    void save( const char *spath, const char *type=NULL ) const;

    // x0, x1, y0, y1 <- [0,1]
    void drawline( float x0, float y0, float x1, float y1, FLOAT3 col );
    void drawrect( float nx0, float ny0, float mx1, float my1, FLOAT3 col );

    //x0, x1,  <- [0,w-1];  y0, y1 <- [0,h-1]
    void drawlineX( int x0, int y0, int x1, int y1, FLOAT3 col );
...
class GPf4
{
  public:
    ...
    void load( int width, int height, const float *a );
...
class GPf1
{
  ...
  private:
    GPf1( const GPf1 &pfm ){}
    GPf1& operator=( const GPf1 &pfm ){return *this;}
};
...
class GPng
{
  public:
    ...
    void load( const char *spath, int num_channel=0 );
    void save( const char *spath ) const;
...
class GPdm
{
  public:
    ...
    void load( int width, int height, const FLOAT3 *fm );
    void load( int width, int height, const DOUBLE3 *fm );
    void load( int width, int height );
    void load( const char *spath );
    void save( const char *spath );
    void flip_vertical();
    DOUBLE3 lookup_linear( double x, double y ) const;
};
...
class GPd4
{
  public:
    ...
    void load( int width, int height, const FLOAT3 *src );
    void load( int width, int height, const DOUBLE4 *src );
    void load( int width, int height );
    void load( const char *spath );
    void save( const char *spath );

    void draw( const GPd4 &blk, int x, int y);
    void getblk( GPd4 &blk, int x, int y, int blkw, int blkh ) const;
    void add( const GPd4 &A );
    void mul( double a );
    DOUBLE4 lookup_linear( double sx, double sy ) const;
    void flip_vertical();
};
