...
public class ExportAnimEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        _animation = (Animation)EditorGUILayout.ObjectField("Model：", _animation, typeof(Animation), false);   
        if(_animation == null)
        {
            EditorGUILayout.HelpBox("Please select a prefab with an Animation component", MessageType.Error);
            return;
        }
        GUILayout.Space(5);
        ...
        if (index == 0)
        {
            EditorGUILayout.HelpBox("Animation list is empty!", MessageType.Error);
            return;
        }
        GUILayout.Space(5);
        _animFrame = (AnimFrame)EditorGUILayout.EnumPopup("Animation Frame Rate:", _animFrame);

        GUILayout.Space(5);
        if (GUILayout.Button("Export"))
        {
            string dir = EditorUtility.SaveFolderPanel("Export animation data", "", "");
